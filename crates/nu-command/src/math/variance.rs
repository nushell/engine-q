...
fn sum_of_squares(values: &[Value], span: &Span) -> Result<Value, ShellError> {
    let n = Value::Int { val: values.len() as i64, span: *span };
    let mut sum_x = Value::Int { val: 0, span: *span };
    let mut sum_x2 = Value::Int { val: 0, span: *span };
    for value in values {
        let v = match &value {
            Value::Int { .. }
            | Value::Float { .. } => {
            ...
            _ => Err(ShellError::UnsupportedInput(
                    ...
                    value.span().unwrap_or(Span::unknown()),
                ))
        }?;
    ...
    let sum_x_squared_div_n = sum_x_squared.div(*span, &n.into())?;
...
pub fn compute_variance(sample: bool) -> impl Fn(&[Value], &Span) -> Result<Value, ShellError> {
    ...
    let ss = match sum_of_squares {
        Err(ShellError::UnsupportedInput(_, err_span)) => Err(ShellError::UnsupportedInput(
                "Attempted to compute the variance with a item that cannot be used for that.".to_string(),
                err_span,
            )),
        other => other
    }?;
    let n = Value::Int { val: n as i64, span: *span };
    ss.div(*span, &n.into())
    }
}
