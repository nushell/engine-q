...
use nu_protocol::{
    Category, SyntaxShape, PipelineData, ShellError,
...
impl Command for FromOds {
    ...
    fn run(
        ...
        let sel_sheets = if let Some(Value::List { vals: columns, .. }) = call.get_flag(engine_state, stack, "sheets")? {
...
fn collect_binary(input: PipelineData) -> Result<Vec<u8>, ShellError> {
    ...
    loop {
        match values.next() {
            ...
            Some(x) => return Err(ShellError::UnsupportedInput(
...
fn from_ods(input: PipelineData, head: Span, sel_sheets: Vec<String>) -> Result<PipelineData, ShellError> {
    let bytes = collect_binary(input)?;
    let buf: Cursor<Vec<u8>> = Cursor::new(bytes);
    let mut ods = Ods::<_>::new(buf).map_err(|_| {
        ShellError::UnsupportedInput("Could not load ods file".to_string(), head)
    })?;
    ...
    for sheet_name in &sheet_names {
        ...
        if let Some(Ok(current_sheet)) = ods.worksheet_range(sheet_name) {
            for row in current_sheet.rows() {
                ...
                for (i, cell) in row.iter().enumerate() {
                    let value = match cell {
                        ...
                        DataType::Float(f) => Value::Float{ val: *f, span: head },
                        DataType::Int(i) => Value::Int {val: *i, span: head },
                        DataType::Bool(b) => Value::Bool { val: *b, span: head },
            ...
            dict.insert(sheet_name, Value::List { vals: sheet_output, span: head });
        } else {
